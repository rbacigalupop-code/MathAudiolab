...
export default function ModoSumas({ store, setStore, audio, instrumento, setRockActive, rockActive }) {
  ...
  const cfg = NIVELES_SUMAS[nivelSeleccionado - 1];
  ...
  const newQ = useCallback(() => {
    const weighted = getWeightedProblem("sums");
    let n1, n2;
  ...
  const check = useCallback(async () => {
    ...
    const updatedStoreFromRecord = recordAttempt("sums", num1, num2, isCorrect);
        ...
        {levelUpMsg && (
          ...
          >
            ...
            <div style={{ fontSize: "clamp(11px, 2vw, 13px)", opacity: 0.85 }}>
              {NIVELES_SUMAS[store.nivel - 1].desc}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
